...
class DownloadRepositoryTest {
    ...
    fun enqueue_invalid_media_throws() =
        runTest {
            val fakeScheduler = FakeWorkScheduler()
            val repository = WorkManagerDownloadRepository(fakeScheduler)
    ...
    private fun mediaItem(mediaUrl: String?) =
        MediaItem(
            id = "media-1",
            accountId = "account-1",
            caption = "caption",
    ...
    private fun downloadRecord(retryUrl: String?) =
        DownloadRecord(
...
private class FakeWorkScheduler : WorkScheduler {
    ...
    override fun getWorkInfosByTagLiveData(tag: String): LiveData<List<WorkInfo>> = MutableLiveData(emptyList())
